...
<template lang="pug">
    .main-blood
        .tabs
            .per-tab(v-bind:class="index == 0 ? 'on':''" @click="handleIndex(0)") 血液录入 
            .com-tab(v-bind:class="index == 1 ? 'on':''" @click="handleIndex(1)") 血液送检
        .tab-item1(v-if="index == 0")
            ...
            .form-content
                <el-table stripe :data="items" style="width: 100%">
                    ...
                    <el-table-column prop="state" label="血液状态">
        ...
        .tab-item2(v-if="index == 1") 
            ...
            <el-form :model="ruleForm" ref="ruleForm" :inline="true" label-width="80px" class="check-group">
                <el-form-item>
                    <el-button type="primary" size="small" @click="handleCheck">送检</el-button>
...
export default {
    ...
    methods: {
        ...
        handleList(params=this.ruleForm) {
            // 获取肛试送检列表
            this.$post('/examination/BloodList', qs.stringify(params),{'Content-Type':'application/x-www-form-urlencoded'}).then(data => {
                this.items = data
